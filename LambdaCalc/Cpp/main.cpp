...
enum class ExprType { Lambda, Application, Variable };

enum class TokenType { Lbracket, Rbracket, Dot, Lambda, Variable };
...
struct Expression {
    ...
    Expression(const Expression &other) : type(other.type) {
        ...
        case ExprType::Application:
            applicationPtr =
                std::make_unique<Application>(*(other.applicationPtr));
            break;
...
struct Lambda {
    ...
    Lambda() {}
    Lambda(const std::string arg, const Expression &body)
        : arg(arg), body(body) {}
...
struct Parser {
    ...
    bool Parse(Expression *result) {
        ...
        if (tokens[pos++].type == TokenType::Lbracket && Parse(&tempExpr1) &&
        ...
        if (tokens[pos++].type == TokenType::Lambda && Parse(&tempExpr1) &&
            tempExpr1.type == ExprType::Variable &&
            tokens[pos++].type == TokenType::Dot && Parse(&tempExpr2)) {
            std::clog << "Found lambda\n";
            result->type = ExprType::Lambda;
            result->lambdaPtr =
                std::make_unique<Lambda>(tempExpr1.variable, tempExpr2);
            return true;
        }
        ...
        if (Parse(&tempExpr1) && tempExpr1.type == ExprType::Lambda &&
            Parse(&tempExpr2)) {
