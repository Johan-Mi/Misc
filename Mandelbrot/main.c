...
#define WIDTH ((uint32_t)8192)
#define HEIGHT ((uint32_t)8192)
#define FILL_COLOR 0xffff000000000000
#define BLANK_COLOR 0xffffffffffffffff

int main(void) {
    uint32_t i, j;
    uint64_t *pic = (uint64_t *)malloc(WIDTH * HEIGHT * sizeof(uint64_t));
    FILE *fp;

    for (i = 0; i < HEIGHT; i++) {
        for (j = 0; j < WIDTH; j++) {
            long double x = j * 4.0l / WIDTH - 2.0l;
            long double y = i * 4.0l / HEIGHT - 2.0l;
