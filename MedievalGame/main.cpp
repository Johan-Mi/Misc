...
enum class TileType : uint8_t { Normal, Wall };
...
void renderMap(sf::RenderWindow &window) {
    for (int i = 0;
         i < screenHeight / 16 + (cam.y != map->height - screenHeight / 16);
         i++) {
        for (int j = 0;
             j < screenWidth / 16 + (cam.x != map->width - screenWidth / 16);
             j++) {
            sf::VertexArray quad(sf::Quads, 4);
            quad[0].position =
                sf::Vector2f(j * 16 - cam.xSub, i * 16 - cam.ySub);
...
void loadAllMaps() {
    ...
    while (mapNames >> currentName) {
        std::ifstream currentMap(
            "Maps/" + currentName, std::ios::in | std::ios::binary
        );
        std::vector<unsigned char> contents(
            (std::istreambuf_iterator<char>(currentMap)),
...
void renderPlayer(sf::RenderWindow &window) {
    ...
    playerSprite[0].position = sf::Vector2f(
        (player.x - cam.x) * 16 + player.xSub - cam.xSub,
        (player.y - cam.y - 1) * 16 + player.ySub - cam.ySub
    );
