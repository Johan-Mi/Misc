...
Float3d
triNormal(Float3d const *const, Float3d const *const, Float3d const *const);
...
int main(void) {
    ...
    window = SDL_CreateWindow(
        "3D", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH,
        ...
        for (size_t i = 0; i < numTris; i++) {
            ...
            if (zClipTri(&p1, &p2, &p3, &p4)) {
                ...
                float colorMultiplier =
                    1.0f - cbrtf(fabsf(normal.x + normal.y)) * 0.05f;
                glColor3f(
                    tris[i].color.r * colorMultiplier,
                    tris[i].color.g * colorMultiplier,
                    tris[i].color.b * colorMultiplier
                );
                glVertex3f(
                    p1.x, p1.y * SCREEN_WIDTH / SCREEN_HEIGHT, 1 - 1 / p1.z
                );
                glVertex3f(
                    p2.x, p2.y * SCREEN_WIDTH / SCREEN_HEIGHT, 1 - 1 / p2.z
                );
                glVertex3f(
                    p3.x, p3.y * SCREEN_WIDTH / SCREEN_HEIGHT, 1 - 1 / p3.z
                );
                if (p4.z == p4.z) {
                    glVertex3f(
                        p1.x, p1.y * SCREEN_WIDTH / SCREEN_HEIGHT, 1 - 1 / p1.z
                    ...
                    glVertex3f(
                        p4.x, p4.y * SCREEN_WIDTH / SCREEN_HEIGHT, 1 - 1 / p4.z
                    );
                }
            }
        }
...
bool zClipLine(Float3d *p1, Float3d *p2) {
    ...
    *p1 = (Float3d
      ...
      (p1->y * f - p2->y * (f - 1.0f)) / (p2->z - p1->z), 0.0f};
    return true;
}
    ...
    if (fp) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            if (line[0] == 'v' && line[1] == ' ') {
                ...
                points[numPoints++] = (Float3d){f1, f2, f3};
            } else if (line[0] == 'f' && line[1] == ' ') {
                size_t s1, s2, s3, s4, unused = 0;
                int p = sscanf(
                    line + 2, "%lu/%lu/%lu %lu/%lu/%lu %lu/%lu/%lu %lu", &s1,
                    &unused, &unused, &s2, &unused, &unused, &s3, &unused,
                    &unused, &s4
                );
                tris[numTris++] =
                    (Tri){s1 - 1, s2 - 1, s3 - 1, {0.7f, 0.7f, 0.7f}};
                if (p > 9) {
                    tris[numTris++] =
                        (Tri){s1 - 1, s3 - 1, s4 - 1, {0.7f, 0.7f, 0.7f}};
                }
            }
        }
        fclose(fp);
...
Float3d triNormal(
    Float3d const *const p1, Float3d const *const p2, Float3d const *const p3
) {
    Float3d v = {
        p2->x - p1->x,
        p2->y - p1->y,
